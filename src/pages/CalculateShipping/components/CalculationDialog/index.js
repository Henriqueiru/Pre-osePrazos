...
function CalculationDialog({ close, apiData, objectType, ...rest }) {
  ...
  useMemo(() => {
    const { packagingKey } = apiData.formData;
    ...
    if (packagingKey) {
      packaging.datasets.forEach((data) => {
        if (data.id === packagingKey) {
          packagingValue = data.price;
        }
      });
    }

    setPackingValue(packagingValue);
  }, [apiData, packaging]);
  ...
  function mountSharedMessage() {
    const { address, services } = apiData.api;
    ...
    data.title = `Cotação feita em ${moment().format('DD/MM/YYYY')} \nDe ${
      address.origin.zipCode
    } para ${address.destination.zipCode}\n\n`;
  ...
  function calculateTotalValueOfService(serviceValue) {
    ...
    return currencyFormatter(
      strValue + parseInt(packingValue, 10) + parseInt(handlingFee, 10)
    );
  }

  function renderServiceItems(data) {
    const { generalSetup } = setup;
    const { additionalService } = apiData.formData;
    const { origin, destination } = apiData.api.address;
    ...
    if (
      origin.Logradouro === 'CEP inexistente' ||
      destination.Logradouro === 'CEP inexistente'
    ) {
    ...
    return data.map((value) => {
              ...
              <AccordionDetails className={classes.accordionDetails}>
                {value.Valor !== '0,00' ? (
                  <>
                    ...
                    {objectType === 1 && value.ValorRegistro !== '0,00' && (
                      <div className={classes.accordionDetailsItems}>
                        <Typography>
                          {additionalService.lowcostRegistry
                            ? 'Registro Módico'
                            : 'Registro Nacional'}
                        </Typography>
                    ...
                    {additionalService.acknowledgmentOfReceipt && (
                      <div className={classes.accordionDetailsItems}>
                    ...
                    {additionalService.valueStatement && (
                      <div className={classes.accordionDetailsItems}>
                    ...
                    {packingValue !== '0' && (
                      <div className={classes.accordionDetailsItems}>
                        ...
                        <Typography>
                          {currencyFormatter(packingValue)}
                        </Typography>
                      </div>
                    )}

                    {generalSetup.handlingFee !== '0' && (
                      <div className={classes.accordionDetailsItems}>
                        <Typography>Taxa de Manuseio</Typography>

                        <Typography>
                          {currencyFormatter(generalSetup.handlingFee)}
                        </Typography>
                      </div>
                    )}
      ...
      if (
        isSedexService1012.length === 0 &&
        serviceCode[0][value.Codigo] !== 'SEDEX 10'
      ) {
        return (
          <Grid item key={value.Codigo} xs={12}>
            <Accordion elevation={0}>
      ...
      <>
        <Box mt={2}>
          ...
          <Typography>
            ...
            {origin.Cidade && `${origin.Cidade}/${origin.Estado}`}
          </Typography>
        </Box>
  ...
  return (
    ...
    >
      ...
      <DialogContent className={classes.dialogContent}>
        ...
        <Grid container>{renderServiceItems(apiData.api.services)}</Grid>

        {renderServiceAddress(apiData.api.address)}
      </DialogContent>
      ...
      <div id="capture" style={{ display: 'none' }}>
        ...
        <DialogContent className={classes.dialogContent}>
          ...
          <Box mt={2}>
            <Typography>
              <strong>ORIGEM: </strong>
              {apiData.api.address.origin.zipCode}
            </Typography>
          </Box>
          ...
          <Box mt={2}>
            <Typography>
              <strong>DESTINO: </strong>
              {apiData.api.address.destination.zipCode}
            </Typography>
          </Box>
        </DialogContent>
      </div>
